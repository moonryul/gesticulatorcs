...
def main(args):
    ...
    # 1. Load the model
    model = GesticulatorModel.load_from_checkpoint(  # the method from  ModelIO(object):
    ...
    audio = torch.randn(1,550,64)
    text = torch.randn(1,550,773)

    # . Any non-Tensor arguments will be hard-coded into the
    #         exported model; any Tensor arguments will become inputs of the exported model,
    #         in the order they occur in the tuple.

    dummy_input = torch.randn(1, 550, 64, 550, 773, requires_grad=True)  
    # print("dummy_input", dummy_input)
    # # Convert_ONNX()
    torch.onnx.export(model,         # model being run 
         (audio, text, True, None),       # model input (or a tuple for multiple inputs) 
         "gesticulator.onnx",       # where to save the model  
