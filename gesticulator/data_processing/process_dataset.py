...
def _encode_vectors(audio_filename, gesture_filename, text_filename, embedding_model, mode, args, augment_with_context):
    ...
    if mode == 'test':
        ...
        seq_length = args.seq_len # = 40 audio frames = the length of a sample sequence = the size of a sample; the label = the gesture corresponding to a sample sequence
    elif mode == 'dev':
    ...
    if args.feature_type == "MFCC": # audio features: MFCC: https://brightwon.tistory.com/11
        ...
        input_vectors = tools.calculate_spectrogram(audio_filename) # mel scale: https://en.wikipedia.org/wiki/Mel_scale
    ...
    # Subsample motion (from 60 fsp to 20 fps): 60 frames per second in bvh file corresponds to Frame Time:	0.0166667
    ...
    n_reserved_inds = seq_length + args.future_context  # seq_length = 40 frames
    ...
    input_vectors_final  = np.array([input_vectors[i - args.past_context : i + n_reserved_inds] 
                                     for i in range(start_ind, stop_ind, seq_step)])  # seq_step = 0.5s = 5 frames = 40 frames => shape=(1251,70,64)
    ...
    output_vectors_final = np.array([output_vectors[i - args.past_context : i + n_reserved_inds]
                                     ...
                                     # shape = (1251, 70, 45), where 45 = the number of joint angles;
                                     # refer to:   ges_str = np.load(gesture_filename);    output_vectors = ges_str['clips']
    ...
    text_vectors_final   = np.array([text_encoding[i - args.past_context // 2 : i + n_reserved_inds // 2]
                                     for i in range(start_ind // 2, stop_ind, seq_step // 2)]) 
                                       # shape = (1251, 35, 773), where 773 is the dimension of the feature vector of each frame
                                       # Each word vector of 733 dim will be repeated according to the number of audio frames for the word token
...
def _save_data_as_sequences(data_csv, save_dir, embedding_model, dataset_name, args):
    ...
    for i in tqdm.trange(len(data_csv)): # the length of the training samples = 19 out of the total of 23 samples
...
def _save_dataset(data_csv, save_dir, embedding_model, dataset_name, args):
    ...
    for i in tqdm.trange(len(data_csv)): # = 19 training samples selected from the total of 23 samples
        text_file = data_csv['wav_filename'][i][:-3] + "json"
    ...
    np.save(x_save_path, X) # shape = (19338, 70, 64)
    np.save(t_save_path, T) # shape = (19338, 35, 773)
    np.save(y_save_path, Y) # shape = (19338, 79, 45)
