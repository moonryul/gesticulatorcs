...
# For this work, we specifically  used the Trinity Gesture Dataset [13], comprising 244 minutes of
# audio and motion capture recordings of a male actor speaking freely  on a variety of topics. 
...
#  Fingers were not modelled  due to poor data quality
     ...
     #MJ:  Speech-encoding dimensionality 124 at each of 30 frames, producing 3720 elements after concatenation.

        self.audio_dim = self[0]['audio'].shape[-1] #self refers to a DataSet object, which is an array
...
class ValidationDataset(Dataset):
    ...
    def __init__(self, root_dir, past_context, future_context):
        ...
        # Get the data
        self.audio = np.load(path.join(root_dir, 'dev_inputs', 'X_dev_NaturalTalking_001.npy')).astype(np.float32) ## data.csv versus data.npy
