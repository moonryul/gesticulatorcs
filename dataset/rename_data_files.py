...
if __name__ == '__main__':
    ...
    parser.add_argument('--folder_downloaded', '-f_down', default="./genea_data", #default=".\\genea_data" 
                                   help="Path where the GENEA dataset is stored, downloaded from the `GENEA_Challenge_2020_data_release` folder.")
    parser.add_argument('--folder_renamed', '-f_renamed',  default="./raw_data", #default=".\\genea_data", 
    ...
    with open('.\\Data_renaming_indices.csv') as csvfile:
         ...
         for row in csv_reader:
             ...
             source_file_name = join(audio_genea_folder, genea_file_name)

           
             #source_file_name = join(cwd, source_file_name)

             #open("D:\\Dropbox\\metaverse\\gesticulator\\dataset\\genea_data\\Training_data\\Audio\\Recording_001.wav")
             #open(source_file_name)
             target_file_name =  join(audio_original_folder, original_file_name)
             #target_file_name = join( cwd, target_file_name)

             #open(target_file_name)
             os.rename( source_file_name, target_file_name ) # Change only the uppermost part
